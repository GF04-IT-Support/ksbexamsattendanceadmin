...
export default function EditStaffDetailsModal({ staffDetails, isOpen, onClose, mutate, uniqueRoles }: EditStaffDetailsModalProps) {
    ...
    const hasChanges = () => {
     ...
     const getChangedValues = () => {
        ...
        for (const key in editedDetails) {
            if (key !== 'exam_id' && editedDetails[key as keyof StaffDetails] !== staffDetails[key as keyof StaffDetails]) {
                changes[key as keyof StaffDetails] = editedDetails[key as keyof StaffDetails];
            }
        }
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (response.message === 'Staff details updated successfully') {
                ...
                mutate();
                onClose();
            }else if(response.message === 'An error occurred while updating the staff details.'){
    ...
    return (
        ...
        >
            <ModalContent>
                <ModalHeader className="flex flex-col gap-1">Edit Staff Details</ModalHeader>
                <ModalBody>
                    ...
                    <CardContent>
                        ...
                        <Grid item xs={6}>
                            <TextField
                                ...
                                onChange={(e) => setEditedDetails({ ...editedDetails, staff_name: e.target.value })}
                                variant='standard'
                            />
                        </Grid>
                        ...
                        <Grid item xs={6}>
                            <Select
                                ...
                                onChange={(e) => setEditedDetails({ ...editedDetails, staff_role: e.target.value as string })}
                                >
