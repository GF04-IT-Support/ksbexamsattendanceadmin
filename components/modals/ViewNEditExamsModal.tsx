...
type EditableFields = 'date' | 'exam_code' | 'start_time' | 'end_time' | 'venue' | 'year';

const ViewNEditModal  = ({ isOpen, onClose, selectedExam, mutate }: ViewNEditModalProps) => {
    ...
    const getChangedValues = () => {
        ...
        for (const key in editedExams) {
            if (key !== 'exam_id' && editedExams[key as keyof Exam] !== selectedExam[key as keyof Exam]) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if(response?.message === 'The exam schedule has been updated successfully!') {
                toast.success(response.message);
                mutate();
                onClose();
            }else if(response?.message === 'An error occurred while updating the exam schedule.') {
    ...
    const EditableField = ({ isEditing, value, onEdit, onToggleEdit, isMultiple, isDate }: EditableFieldProps) => {
    if (isMultiple) {
        return (
        <Grid item xs={6} className="flex gap-4">
            ...
            {isEditing ? (
                ...
                value.split(',').map((val, index) => (
                <Typography key={index} color="textPrimary">{val.trim()}</Typography>
                ))
            )}
            </div>
            ...
            <div className="flex gap-5 items-center">
                <AiOutlineClose className="cursor-pointer hover:opacity-60" onClick={onToggleEdit}/>
                <AiOutlineCheck className="cursor-pointer hover:opacity-60" onClick={onToggleEdit}/>
            </div>
            ) : (
            <FiEdit2 className="cursor-pointer hover:opacity-60" onClick={onToggleEdit}/>
            )}
        </Grid>
        );
    } else if (isDate) {
        ...
        <Grid item xs={6} className="flex gap-4">
            ...
            <Typography color="textPrimary">{new Date(value).toLocaleDateString()}</Typography>
            )}
                    ...
                    <CardContent>
                        ...
                        <Grid item xs={6}>
                            ...
                            <EditableField
                                ...
                                onEdit={(newValue) => setTempValues({ ...tempValues, exam_code: newValue })}
                        ...
                        <Grid item xs={6}>
                            ...
                            <EditableField
                                ...
                                onEdit={(newValue) => setTempValues({ ...tempValues, date: newValue })}
                        ...
                        <Grid item xs={6}>
                            ...
                            <EditableField
                                ...
                                onEdit={(newValue) => setTempValues({ ...tempValues, start_time: newValue })}
                        ...
                        <Grid item xs={6}>
                            ...
                            <EditableField
                                ...
                                onEdit={(newValue) => setTempValues({ ...tempValues, end_time: newValue })}
                        ...
                        <Grid item xs={6}>
                            ...
                            <EditableField
                                ...
                                onEdit={(newValue) => setTempValues({ ...tempValues, venue: newValue })}
                        ...
                        <Grid item xs={6}>
                            ...
                            <EditableField
                                ...
                                onEdit={(newValue) => setTempValues({ ...tempValues, year: newValue })}
